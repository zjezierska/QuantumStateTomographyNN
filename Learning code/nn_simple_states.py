...
for i in range(0, int(n/2)):
    ...
    infidel1.append(1 - qt.fidelity(pred_res, true_y))
    infidel2.append(1 - qt.fidelity(pred_res1, true_y))

import numpy as np
n = 20000
x_plot = np.linspace(0, int(n/2), int(n/2))
print(x_plot)
plt.plot(x_plot, infidel1, label='normalized')
plt.plot(x_plot, infidel2, label='not normalized')
plt.yscale('log')
plt.legend()
